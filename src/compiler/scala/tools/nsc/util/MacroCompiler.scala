...
abstract class JitCompiler extends Traces {
  ...
  private def createClosure(toCheck:List[g.Symbol],checkedSyms:jitInternalGlobal.SymbolSet,forbiddenSet:Set[g.Symbol]=Set()):jitInternalGlobal.SymbolSet = {
    ...
    //println(g.showRaw(g.currentRun.units.next.body,printIds=true,printTypes=true))
...
abstract class JitInternalGlobal(currentSettings: Settings, reporter: Reporter) extends Global(currentSettings, reporter) with Traces {
              ...
              def processRetType(ddef:DefDef,s:Symbol) = {
                s.info match {
                  ...
                  case mt @ MethodType(params,resultType) =>
                    ...
                    // ddef.vparamss.flatten zip mt.paramss.flatten foreach { case (valdef,sym) => valdef.setSymbol(sym) }
                    // println("!!!processRetType2 " + showRaw(ddef,printTypes=true) + " params:" + mt.paramss)
                    // println("!!!processRetType2 resultType=" + showRaw(resultType))
                    // println("!!!processRetType2 mt=" + showRaw(mt))
    ...
    private abstract class PathWalker(wp:WalkPath,debug:Boolean=false) extends Traverser {
      ...
      private class ContextCreator(unit:CompilationUnit,wp:WalkPath,debug:Boolean=false) extends PathWalker(wp,debug) {
        ...
        def markNode(tree:Tree):Unit  = {
          ...
          //println("!!!makeContext based on " + tree.symbol + "  " + tree.getClass)
      ...
      */
      ...
      override def traverse(tree: Tree): Unit  = {
        ...
        def tryToEnter0(s:Symbol) = if ( isExternal(s) && ifDefinedInTree(s) ) {
          debugEnter(s,tree)
          // Can be flyed over symbol already not in our SCOPE
          assert(isExternal(s) && ifDefinedInTree(s),s"nonstatic external symbol $s have suspicious owner" )
          externalSyms+=s
    ...
    /** SNIPETTS
      ...
      def ll(s: => String,t:Tree=tree) = tree match {
        case f @ DefDef(_,name,_,_,_,_) if name == nme.CONSTRUCTOR => println(s + " " + showRaw(t,printTypes=true) )
        case _ => 
      }
    ...
    **/
    ...
    def compileClosure(closure:SymbolSet,compiler:JitGlobal):JitResult = {
      ...
      val unitsToCompile = closure.groupBy(_.unit).map(gr => {
        ...
        // TODO: generate faced filename to help inner macros
        val cu = new compiler.CompilationUnit(NoSourceFile)
        /*
        val tt = pdef.asInstanceOf[PackageDef].stats(1).asInstanceOf[ModuleDef].impl.body(1).asInstanceOf[DefDef].vparamss(0)(0)
        val a = hardResetAllAttrs(tt,false)
        println("----------------------------")
        println(showRaw(tt,printTypes=true))
        println(showRaw(a,printTypes=true))
        println("----------------------------")
        */
        cu.body = compiler.PackageDef(
          compiler.Ident(pdef.name.toString/*compiler.packageName*/) , wrappedPackage 
        )
        
        //println(compiler.showRaw(cu.body,printTypes=true,printIds=true))
      ...
      if (! hasNewErrors)
        JitSuccess(compiler.packageName)
      else
        JitError()
    }
  ...
  private def packUnitElementsList(unit:CompilationUnit,startTrees:List[Tree],els:List[SymbolPath],startTree:Tree):Iterable[Tree] = {
    ...
    val ret = for ( (tree,i) <- startTrees.zipWithIndex ) yield {
        //println("!!!packUnitElementsListEl i:"+ i + " " + tree.summaryString + " " + simPaths.get(i) )
        simPaths.get(i) match {
          ...
          case None => tree match {
            ...
            // use old solution TODO: refactor first entry point in getLinkedIds
...
object JitGlobal {
  var pkgIdx = 0
  def currentPackageName = "macroimpl"+pkgIdx+"$"
  //lazy val virtualDirectory = new VirtualDirectory("(memory)", None)  
}
...
class JitGlobal(currentSettings: Settings, reporter: Reporter)
  ...
  // TODO: Race condition ???
  JitGlobal.pkgIdx+=1
  val packageName = JitGlobal.currentPackageName

}

abstract class JitResult
case class JitSuccess(packageName:String) extends JitResult
